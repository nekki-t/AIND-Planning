...
class PlanningGraph():
    ...
    def add_action_level(self, level):
        ...
        for action in self.all_actions:
            ...
            if action_node.prenodes.issubset(previous_s):
                self.a_levels[level].add(action_node)
                for state_node in previous_s:
    ...
    def add_literal_level(self, level):
        ...
        previous_a = self.a_levels[level - 1]
        for action_node in previous_a:
            for eff_s_node in action_node.effect_s_nodes():
                self.s_levels[level].add(eff_s_node)
                action_node.children.add(eff_s_node)
                eff_s_node.parents.add(action_node)
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        for a1_effnode in node_a1.effnodes:
            for a2_prenode in node_a2.prenodes:
                if self.negation_mutex(a1_effnode, a2_prenode):
                    return True
        for a1_prenode in node_a1.prenodes:
            for a2_effnode in node_a2.effnodes:
                if self.negation_mutex(a1_prenode, a2_effnode):
                    return True
    ...
    def competing_needs_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        for a1_precon in node_a1.parents:
            for a2_precon in node_a2.parents:
                if a1_precon.is_mutex(a2_precon):
                    return True
        return False
    ...
    def inconsistent_support_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s):
        ...
        for s1_parent_action in node_s1.parents:
            for s2_parent_action in node_s2.parents:
                if not s1_parent_action.is_mutex(s2_parent_action):
                    return False
        return True
